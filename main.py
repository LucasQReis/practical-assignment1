...
for file in files:
    with open(file, "r", encoding="utf-8") as f:
        ...
        for d in discussions:
            d["_source_file"] = file_labels[file]
...
def avg_plausibility(row):
    theories = row.get("analysis.theory")
    scores = []
    if isinstance(theories, list):
        scores = [t["plausibility"] for t in theories if isinstance(t, dict) and t.get("has_theory") and "plausibility" in t]
    elif isinstance(theories, dict):
        fan = theories.get("fan_theories", [])
        scores = [t["plausibility"] for t in fan if isinstance(t, dict) and t.get("has_theory") and "plausibility" in t]
    return round(sum(scores) / len(scores), 2) if scores else None

def max_plausibility(row):
    theories = row.get("analysis.theory")
    scores = []
    if isinstance(theories, list):
        scores = [t["plausibility"] for t in theories if isinstance(t, dict) and t.get("has_theory") and "plausibility" in t]
...
df_clean["avg_theory_plausibility"] = df_clean.apply(avg_plausibility, axis=1)
df_clean["max_theory_plausibility"] = df_clean.apply(max_plausibility, axis=1)
...
df_clean["timestamp_dt"] = pd.to_datetime(df_clean["timestamp"], utc=True, errors="coerce")

df_clean["published_hour"]     = df_clean["published_dt"].dt.hour
df_clean["published_dayofweek"]= df_clean["published_dt"].dt.dayofweek  # 0=Mon
df_clean["published_month"]    = df_clean["published_dt"].dt.month

min_date = df_clean["published_dt"].min()
df_clean["days_since_first_post"] = (
    df_clean["published_dt"] - min_date
).dt.days
...
new_numeric_cols = [
    "discussion_id", "source_file_id", "sentiment_encoded", "emotion_encoded",
    "avg_theory_plausibility", "max_theory_plausibility",
    "num_themes", "num_canonical_themes", "title_freq",
    "theme_character_development", "theme_nostalgia", "theme_plot_elements",
    "theme_relationships", "theme_theories", "theme_cinematography", "theme_music",
    "published_hour", "published_dayofweek", "published_month",
    "days_since_first_post", "crawl_lag_hours",
    "summary_length", "title_length", "engagement_score",
]

df_clean.to_csv("cleaned_dataset_enriched.csv", index=False)